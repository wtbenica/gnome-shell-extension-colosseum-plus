 ...
 */
/**
 * Centralized error handling utilities for the Colosseum extension.
 *
 ...
 */
function logMessage(message, context, level = "error") {
    const fullMessage = context
        ? `[Colosseum] ${context}: ${message instanceof Error ? message.message : String(message)}`
        : `[Colosseum] ${message instanceof Error ? message.message : String(message)}`;
    switch (level) {
        case "error":
...
export function logFile(message, filename) {
    try {
        ...
        if (logFile.query_exists(null)) {
            ...
            logFile.replace_contents(logLine, null, false, Gio.FileCreateFlags.REPLACE_DESTINATION, null);
        }
    }
    catch (e) {
