...
export function logFile(endpoint, params = {}) {
    ...
    try {
        ...
        apiLogFile.replace_contents(newContent, null, false, Gio.FileCreateFlags.NONE, null);
    }
    catch (e) {
...
function logToFile(message, level) {
    ...
    try {
        ...
        if (existingContent.length > 1024 * 1024) {
            ...
            backupFile.replace_contents(existingContent, null, false, Gio.FileCreateFlags.NONE, null);
            levelLogFile.replace_contents(message + '\n', null, false, Gio.FileCreateFlags.NONE, null);
        }
        else {
            const newContent = existingContent + message + '\n';
            levelLogFile.replace_contents(newContent, null, false, Gio.FileCreateFlags.NONE, null);
        }
    }
    catch (e) {
        // Avoid calling logInfo here to prevent recursion (logInfo -> logToFile -> error)
